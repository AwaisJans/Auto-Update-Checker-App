...
class RecyclerItemTouchHelper(private val adapter: ToDoAdapter) :
    ...
    override fun onChildDraw(
        ...
        if (dX > 0) {
            icon = ContextCompat.getDrawable(adapter.context, android.R.drawable.ic_menu_edit)
            background = ColorDrawable(ContextCompat.getColor(adapter.context, R.color.black))
        } else {
            icon = ContextCompat.getDrawable(adapter.context, android.R.drawable.ic_delete)
